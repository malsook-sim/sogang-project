...
import { Heart, Play, Pencil, Trash, Mic } from "@/components/Icon";
import PageHeader from "@/components/PageHeader";
import { StoryCover } from "@/components/StoryCover";
import { VoiceAvatar } from "@/components/VoiceAvatar";
import { moralKeywords, moralSummaryLines } from "@/lib/morals";
...
export default function StoryDetailPage() {
  ...
  const summaryLines = moralSummaryLines(story.morals, story.moralSummary, 2);
  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto px-5 lg:px-8 pt-6 pb-28 lg:pb-6 lg:grid lg:grid-cols-[38%_1fr] lg:gap-8">
        {/* 좌: 표지 + 제목 + 태그 + 교훈 + CTA */}
        <div className="lg:sticky lg:top-24 lg:self-start mb-6 lg:mb-0">
          ...
          {editing ? (
            <input
              ...
              className="w-full h-11 px-3.5 mb-3 rounded-[10px] bg-background border border-border text-[16px] font-bold text-foreground focus:outline-none focus:border-[1.5px] focus:border-primary focus:ring-[3px] focus:ring-primary-light transition"
            />
          ) : (
          ...
          {!editing && (
            <div>
              ...
              {isMine && (
                <button
                  onClick={startEdit}
                  className="w-full mt-2.5 flex items-center justify-center gap-2 border border-border py-3.5 rounded-2xl text-sm font-semibold hover:bg-surface transition"
                >
                  <Pencil size={15} />
                  수정하기
                </button>
          ...
          {editing ? (
            <>
              <textarea
                ...
                className="w-full block rounded-xl bg-background border border-border text-[15px] lg:text-[16px] text-foreground/90 resize-none overflow-hidden focus:outline-none focus:border-[1.5px] focus:border-primary focus:ring-[3px] focus:ring-primary-light transition"
      ...
      {!editing && (
        <div className="lg:hidden fixed left-0 right-0 z-40 bottom-[calc(68px+env(safe-area-inset-bottom))] glass border-t border-border px-4 py-3">
          {lastVoice ? (
      ...
      {voiceModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
          ...
          />
          <div className="relative z-10 w-full sm:max-w-md bg-surface rounded-t-2xl sm:rounded-2xl shadow-xl max-h-[80vh] flex flex-col">
            ...
            <div className="overflow-y-auto p-2.5">
              {clonedVoices.length > 0 && (
                <p className="px-2 pt-1 pb-1 text-[11px] font-bold text-muted uppercase tracking-wider">
                  내 목소리
                </p>
              )}
              {voiceList.map((v, i) => {
                ...
                return (
                  <div key={v.id}>
                    {firstBase && (
                      <p className="px-2 pt-2 pb-1 text-[11px] font-bold text-muted uppercase tracking-wider">
                        기본 목소리
                      </p>
                    )}
                    <button
                    ...
                    >
                      <VoiceAvatar emoji={v.emoji} size={34} />
                      <span

...
export default function SleepIndicator() {
  ...
  return (
    <div className="fixed top-3 right-3 z-[45]">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 h-9 pl-2.5 pr-3 rounded-full bg-[#3D3A5C] border border-white/10 text-[var(--star)] text-xs font-bold tabular-nums shadow-lg shadow-black/20 hover:bg-[#474269] transition"
        aria-label="수면 모드"
      >
        <Moon size={15} filled />
        {mins ? `${mins}분` : "수면"}
          ...
          <div className="absolute right-0 top-full mt-2 w-52 bg-[#2C2A45] border border-white/10 rounded-2xl shadow-xl p-3.5">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 py-2 rounded-lg text-sm font-semibold bg-star/15 text-star hover:bg-star/25 transition"
              >
                끄기
              </button>
            </div>
